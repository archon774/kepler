"""Kepler: agentic loop over the ``kepler.tools`` schemas.

Ported from ``database_tools.py``'s ``main()``, updated for
``kepler.tools.registry.TOOL_SCHEMAS`` -- one schema per database instead of
one schema with a ``database`` enum switch, and returning
``kepler.models.ToolResult`` (via ``.model_dump()``) rather than a raw dict.
...
from kepler.tools.registry import TOOL_FUNCTIONS, TOOL_SCHEMAS
...
#:    tool level (kepler.tools.vizier); the rest is strategy tool responses
#:    alone can't convey.
...
VizieR, ATNF, MAST, MPC, CASDA, and ADS (kepler.tools).
