...
class Ucac5Index:
    ...
    Accepts either on-disk layout — the catalog root may be the *parent* of the
    ``u5z`` directory, or the zone directory itself:

      <root>/u5z/u5index.asc, z001 … z900     (standard distribution)
      <root>/u5index.asc, z001 … z900         (root *is* the zone dir)

    Detected by where ``u5index.asc`` is found (the ``u5z`` subdir is tried
    first), so pointing ``SKYLIB_UCAC5_ROOT`` at either path works.
    """
    ...
    def __init__(self, root: str | Path, *, u5z_subdir: str = "u5z", cache_size: int = 4):
        ...
        # Accept either layout: a root that *contains* the standard `u5z/` zone
        # directory, or a root that *is* the zone directory. Detect by where the
        # ASCII index lives (prefer the `u5z` subdir), so pointing the catalog
        # root at `.../UCAC5` or `.../UCAC5/u5z` both resolve.
        subdir = self.root / u5z_subdir
        if (subdir / "u5index.asc").exists():
            self.u5z_dir = subdir
        elif (self.root / "u5index.asc").exists():
            self.u5z_dir = self.root
        else:
            raise FileNotFoundError(
                "UCAC5 catalog not found: looked for 'u5index.asc' in "
                f"{subdir} and {self.root}. Point the catalog root at the "
                "directory holding the zone files (or its parent)."
            )
