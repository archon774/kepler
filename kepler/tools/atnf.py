...
*full* list (``catalogs.atnf.PSR_ALL_PARAMS``) as the default: confirmed
...
``include_errs``/``include_refs``. ``catalogs.atnf.PSR_ALL_PARAMS`` remains
useful as a reference vocabulary for a caller who wants to request a
deliberate subset (e.g. ``catalogs.atnf.PSR_BINARY_PARAMS``).
...
def search_atnf(
    ...
    subset (e.g. ``catalogs.atnf.PSR_BINARY_PARAMS``) to narrow it. A name
