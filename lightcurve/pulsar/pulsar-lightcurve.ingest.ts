...
//     pulsar-light-curve.component.ts                    (365 lines)
//   src/app/tools/pulsar/light-curve/pulsar-light-curve-form/
//     pulsar-light-curve-form.component.ts               (108 lines, onBackScaleChange only)
//
// Despite living in `*.component.ts`, `uploadHandler` is the pulsar tool's
// real data-ingest algorithm: it discriminates the two Green Bank file
// flavours, pulls the observation metadata out of the '#' header block,
// rebases the time axis onto seconds-since-UTC-start, derives the Nyquist
// sampling limit, and runs the background subtraction. All of that is
// carried over verbatim below.
//
// FRAMEWORK SEAMS CUT:
//   - `@Component({...})` decorator, `@angular/core` and `@angular/material`
//     (MatDialog) imports
//   - the constructor's Angular DI (`private service: PulsarService`,
//     HonorCodePopupService, HonorCodeChartService, MatDialog) — the service
//     is now a plain constructor argument typed against the local
//     `PulsarIngestHost` seam interface below
//   - `actionHandler()` (table/graph toolbar dispatch) and `saveGraph()`
//     (honor-code popup + Highcharts PNG export) — pure UI, not extracted
//
// RETAINED ON PURPOSE: `FileReader`. It is a browser API, not a framework
// dependency, and the entire algorithm lives inside its onload callback.
// Keeping it means the method body is byte-identical to the original.

import {PulsarDataDict} from "./pulsar-lightcurve.types";

/**
 * EXTRACTED: the seam replacing Angular's injected `PulsarService`.
 * Every member below is a method the extracted code actually calls. The
 * light-curve algorithms live in ./pulsar-lightcurve.algorithms.ts; the
 * remaining members are state the ingest writes through to.
 */
export interface PulsarIngestHost {
  // Light-curve math
  ...
  // Period-folding parameters derived from the file
  ...
  // EXTRACTED: UI seams. `clearPeriodogramChart` wipes a Highcharts chart,
  // `setTabIndex` drives tab selection, `setLightCurveOptionValid` gates which
  // tabs are reachable, and the set*Title calls are chart labels. They are
  // declared here only so the verbatim bodies below still compile.
...
// EXTRACTED: was `@Component({...}) export class PulsarLightCurveComponent`
export class PulsarLightCurveIngest {
  ts: number[] = [];
  xs: number[] = [];
  ys: number[] = [];
  ...
  // EXTRACTED: was Angular constructor DI over PulsarService +
  // HonorCodePopupService + HonorCodeChartService + MatDialog.
  constructor(private service: PulsarIngestHost) {}
  ...
  /**
   * EXTRACTED from pulsar-light-curve-form.component.ts:70-103
   * (`PulsarLightCurveFormComponent.onBackScaleChange`), body verbatim.
   *
   * This is the interactive re-subtraction path: it re-runs background
   * subtraction against the RAW data whenever the user changes the window
   * width, and — importantly — refuses window widths narrower than ~2.2×
   * the mean sample spacing, below which the running median degenerates.
   * Kept here rather than in the form because that guard is the algorithm's
   * validity condition, not a UI validator.
   */
  onBackScaleChange(value: number): void {
    if (typeof value === 'number' && !isNaN(value)) {
      let chartData = this.service.getRawData();
      ...
      // Apply background subtraction based on the provided backScale
      const subtractedSource1 = this.service.backgroundSubtraction(jd, source1, value);
      const subtractedSource2 = this.service.backgroundSubtraction(jd, source2, value);
