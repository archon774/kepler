...
//   src/app/tools/shared/data/utils.ts  (278 lines, copied byte-for-byte)
//
// This is the algorithmic heart of the periodogram: the Lomb-Scargle
// transform (error-weighted and unweighted variants), the logarithmic
// frequency/period grid construction, and the ArrMath vector helpers the
// transform is built on. Nothing below has been renamed, reordered, or
// reformatted — the only change to this file is this header.
//
// Framework seams: none. utils.ts had zero Angular/RxJS/Highcharts imports.
// Browser seam (left as-is, deliberately): both lombScargle* functions call
// the DOM `alert()` on a ts/ys length mismatch. That is original behaviour;
// a non-browser host must shim `alert` or the call throws.
//
// Consumers of this file:
//   lombScargleWithError -> variable tool (error bars available)
//   lombScargle          -> pulsar tool   (no per-point error)
//   floatMod             -> period folding in BOTH tools (also used by the
//                           lightcurve extraction; duplication is expected)
