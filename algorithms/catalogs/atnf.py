...
than importing ``psrqpy`` here, so that ``algorithms.catalogs`` keeps the same
zero-query-library-import contract throughout this package -- "nothing here
imports astroquery, psrqpy, or opens a socket."
...
that is the accepted cost of not importing it. ``tools.atnf`` re-checks
