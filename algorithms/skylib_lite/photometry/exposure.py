...
from algorithms.skylib_lite.util.angle import airmass_for_el
...
def exptime_for_mag_and_counts(
    ...
    :param point_source: whether the object is a point or extended source.
    :param mag: object brightness in mag for point sources; surface brightness in mag/arcsec^2 for extended sources.
    :param target_counts: desired electrons per pixel (not including blank level). For a point source this is the
        electron count in the PEAK (brightest) pixel — the one that saturates first; for an extended source it is the
        per-pixel count at the given surface brightness.
    :param sky: sky brightness in magnitudes per arcsec^2.
    :param dark: dark current in electrons per pixel per second.
    :param flux15: flux for a 15th magnitude object in electrons per second: flux15 = 10**(-0.4*(15 - zp)).
    :param pixsize: pixel size in arcsec.
    :param seeing: seeing in arcsec (point-source FWHM). Unused for extended sources.
    :param aper: photometric aperture diameter in arcsec. Unused: `target_counts` is a per-pixel quantity, so both
        branches size by the per-pixel count (the point-source peak pixel, the extended-source surface brightness),
        not an aperture sum.
    ...
    if point_source:
        # `target_counts` is a per-pixel limit (e.g. full-well capacity), so a point source must be sized by its
        # PEAK pixel, not its aperture-summed flux. The peak pixel sits at the PSF centre and collects the fraction
        # of the source's total flux that falls in one pixel — a square of side `pixsize` centred on a Gaussian PSF
        # of FWHM `seeing` (so peak_fraction -> 1 when the PSF is much smaller than a pixel). Sizing by the aperture
        # sum instead under-exposed the peak by the PSF concentration factor (~10x at typical sampling).
        sigma = seeing/2.3548  # PSF sigma in arcsec (FWHM = seeing)
        peak_fraction = erf(pixsize/(2*np.sqrt(2)*sigma))**2  # fraction of total flux in the peak pixel
        counts = flux15*10**(-0.4*(mag - 15))*peak_fraction  # source electrons per second in the peak pixel
        total_counts = counts + background  # + background, electrons per second per pixel
    else:
        counts = flux15*pixsize**2*10**(-0.4*(mag - 15))
        total_counts = counts + background
