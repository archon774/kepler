...
``algorithms.query.config.settings`` before the first query, or pass an object
...
class QuerySettings:
    ...
    * ``VIZIER_CACHE_ENABLED`` — whether astroquery caches responses on disk.
      ...
      ``algorithms.query.vizier``. That rounding is observable, so turning the
    ...
    * ``VIZIER_CACHE_AGE_DAYS`` — entries older than this are deleted the next
    ...
    ``algorithms.catalogs.sdss_catalog``, which derives its ``display_name`` from
    it.
    """
