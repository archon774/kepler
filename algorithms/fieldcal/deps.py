...
``algorithms.photometry`` and ``algorithms.wcs``.  They are re-exposed here as
overridable module-level names so that:

  * ``algorithms.fieldcal`` imports without the remote catalog stack, and
  * a host application wires the real implementations in with one assignment,
    e.g. ``algorithms.fieldcal.deps.run_photometry = my_photometry_runner``.
...
def _missing(name: str, original: str, home: str):
    def _stub(*_args: Any, **_kwargs: Any):
        raise FieldCalDependencyError(
            f"algorithms.fieldcal.deps.{name} has not been provided. "
            f"EXTRACTED: was `{original}` in Skynet; belongs in algorithms.{home}. "
            f"Assign an implementation: `algorithms.fieldcal.deps.{name} = <callable>`."
        )
...
def _default_query_catalogs(*args: Any, **kwargs: Any):
    """Query catalogs through Kepler's ``algorithms.query`` package.

    Unlike the stubs above this has a working default, because ``algorithms.query`` is
    ...
    ``algorithms.fieldcal.deps`` does not pull in astroquery — a caller who supplies
    ...
    cache, a test double::
        ...
        algorithms.fieldcal.deps.query_catalogs = my_query_function
    ...
    Signature: query_catalogs(catalogs, *, wcs=None, ra_hours=None,
               ...
               -> list[CatalogSource]
    """
    from algorithms.query.runner import query_catalogs as _impl
