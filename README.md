...
small installable Python package, extracted astronomy algorithms, a prototype
database-query tool, and an architecture document for turning those pieces into
a coherent tool surface.
...
| `kepler/tools/` | Python tools | First plain Python tool wrappers for WCS description, catalog metadata, reference-band resolution, zero-point solving, and local artifact inspection. |
| `kepler/wcs/` | Extracted Python algorithm | Skynet WCS calibration: source extraction, FITS-header hinting, astrometry.net `solve-field`, ATLAS triangle solving, solution validation, and FITS-header write-back. |
| `kepler/photometry/` | Extracted Python algorithm | Skynet source extraction and aperture photometry using the shared `kepler/skylib_lite/` Skylib subset. |
| `kepler/fieldcal/` | Extracted Python algorithm | Skynet photometric zero-point calibration: catalog-source matching, variable-star filtering, reference-magnitude resolution, and weighted zero-point solving. |
| `kepler/skylib_lite/` | Shared Python support | Consolidated local Skylib subset used by WCS, photometry, and field calibration: astrometry, SEP extraction, background estimation, aperture photometry, FITS helpers, angle math, and statistics. |
| `kepler/catalogs/` | Extracted Python algorithm | Skynet and Afterglow photometric catalog declarations: band tables, filter/colour transforms, column mappings, and the SIMBAD object-type vocabulary for eleven catalogs. Declaration only — no network code. |
| `kepler/query/` | Extracted Python algorithm | Skynet and Afterglow remote catalog access: the VizieR engine, SDSS SkyServer SQL, SIMBAD identifier resolution, astroquery cache handling, filter-aware catalog selection, and WCS-footprint query orchestration. |
| `kepler/lightcurve/` | Extracted TypeScript algorithm | Astromancer pulsar and variable-star light-curve ingestion, transformation, and period-folding logic with Angular/RxJS/Highcharts removed. |
| `kepler/periodogram/` | Extracted TypeScript algorithm | Astromancer Lomb-Scargle periodogram logic, peak/confidence helpers, pulsar range defaults, and periodogram-to-folding coupling. |
| `kepler/hrdiagram/` | Extracted TypeScript algorithm | Astromancer cluster/HR-diagram logic: field-star removal, isochrone matching, extinction offsets, cluster summaries, and result projections. |
...
Kepler/
  ...
  kepler/
    tools/                       # first plain Python tool wrappers
...
The first plain Python tools live under `kepler.tools`:

```python
from kepler.tools.astrometry import describe_image_wcs
from kepler.tools.catalogs import list_photometric_catalogs, resolve_reference_band
from kepler.tools.calibration import solve_zeropoint_from_measurements
from kepler.tools.workspace import describe_artifact, list_artifacts
```
...
from kepler.photometry.pipeline.photometry import run_photometry, perform_photometry
from kepler.photometry.pipeline.source_extraction import run_source_extraction
from kepler.fieldcal import perform_field_calibration, calc_solution
from kepler.query.runner import query_catalogs
from kepler.query.simbad import resolve_simbad
```
...
`kepler.fieldcal.deps` to the implementations from `kepler.wcs` and
`kepler.photometry`. Catalogs are the exception:
`kepler.fieldcal.deps.query_catalogs` already defaults to `kepler.query`.

Catalog metadata and catalog access are separate on purpose. Import
`kepler.catalogs` for band tables and colour transforms — it is pure data and
pulls in no network stack. Import `kepler.query.registry` when you need to
...
see `kepler/query/EXTRACTION.md` §5.1.
