...
//   src/app/tools/cluster/cluster.util.ts  (228 lines, copied byte-for-byte)
//
// The core domain model + photometric constants of the HR-diagram tool:
// the FILTER enum and its catalog groupings, the effective-wavelength table,
// the per-filter standard-view framing table, the Source/Photometry/Astrometry
// data model, the PlotConfig / PlotParams / IsochroneParams shapes that
// parameterise isochrone matching, the haversine angular separation, and
// getExtinction (the Cardelli-Clayton-Mathis A(lambda)/A(V) parameterisation
// that drives reddening).
//
// Nothing below has been renamed, reordered or reformatted. The ONLY changes
// are the two import paths (see seams below).
//
// Framework seams: none in the body. cluster.util.ts had zero Angular / RxJS /
// Highcharts imports; it is pure TypeScript.

// EXTRACTED: was `import {rad} from "../shared/data/utils"` (astromancer
//            src/app/tools/shared/data/utils.ts) -> ./shared/angle.util
import {rad} from "./shared/angle.util";
// EXTRACTED: import path only. NOTE this import is unused in the original
//            file as well; it is preserved here to keep the file verbatim.
