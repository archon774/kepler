// EXTRACTED VERBATIM from astromancer:
//   src/app/tools/cluster/cluster-data.service.util.ts  (133 lines, copied
//   byte-for-byte)
//
// The field-star-removal cut itself lives here. updateClusterFieldSources is
// the actual membership test: an ELLIPTICAL acceptance region in proper-motion
// space (semi-axes a, b from the half-width of the pm_ra / pm_dec selections,
// centred on the midpoint of each) intersected with a parallax-distance
// interval. Everything downstream — the CMD, the isochrone fit, the summary —
// operates on the `fsr` (member) partition this function produces.
//
// Also here: sourceSerialization (raw catalog JSON -> Source[], collecting the
// filter set as it goes), appendFSRResults (a sorted merge-join of the backend
// FSR astrometry onto the existing sources by id), and getStarCountsByFilter /
// sourceByCatalog (per-catalog member/field/unused tallies, including the
// commented-out `every` variant the author left in place).
//
// Nothing below has been renamed, reordered or reformatted. The ONLY changes
// are the three import paths.
//
// Framework seams: none. This file had zero Angular / RxJS / Highcharts
// imports in astromancer.

// EXTRACTED: import paths only.
